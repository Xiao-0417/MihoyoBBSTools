...
def login():
    if not config.config["account"]["cookie"]:
        try:
            login_bbs.login_bbs_web(config.config['account']['username'], config.config['account']['password'])
        except Exception as e:
            log.error("自动获取Cookie失败，请手动填充！")
            config.clear_cookie()
            raise CookieError('No cookie')
    if config.config['account']['stoken'] == "":
        try:
            login_bbs.login_bbs(config.config['account']['username'], config.config['account']['password'])
        except Exception as e:
            log.error("自动获取 Stoken 失败，请手动填入 stoken！")
    ...
    uid = get_uid()
    if uid is None:
        log.warning("cookie 缺少 UID，正在刷新cookie")
        try:
            login_bbs.login_bbs_web(config.config['account']['username'], config.config['account']['password'])
        except Exception as e:
            log.error("Cookie自动刷新失败，请手动更新！")
            config.clear_cookie()
...
def get_stoken(login_ticket: str, uid: str) -> str:
    ...
    if data["retcode"] == 0:
        ...
        log.warning("login_ticket（只有半小时有效期）已失效,正在尝试自动刷新米游社 cookie")
        try:
            login_bbs.login_bbs(config.config['account']['username'], config.config['account']['password'])
        except Exception as e:
...
def get_cookie_token_by_stoken():
    ...
    if data.get("retcode", -1) != 0:
        log.warning("stoken 已失效，正在尝试自动刷新 stoken")
        try:
            login_bbs.login_bbs(config.config['account']['username'], config.config['account']['password'])
        except Exception as e:
            log.error("stoken 刷新失败，请手动获取 stoken")
            config.clear_stoken()
