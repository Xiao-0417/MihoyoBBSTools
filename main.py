...
def main():
    ...
    if config.config["account"]["username"] == "" and config.config["account"]["password"] == "":
        log.warning("请输入账号密码后再运行本项目！")
        return 1, "账号密码未设置！"
    if any([config.config["account"]["cookie"] == "", config.config["account"]["stuid"] == "",
    ...
    if any([login_bbs.login_verify_web() is False, login_bbs.login_verify() is False]):
        login.login()
    # 米游社签到
    ret_code = 0
    return_data = "\n"
